...
!       !!! shw40: special test set SO4 and 40-bin to be 0, initially.
...
!       endif
       ...
       IF(FIRST .or. (.NOT.State_Met%InTroposphere(I,J,L) &
                      .and. InTropo_old(I,J,L)==1)         ) THEN

         Spc(I,J,L,Ind_('H2SO4G')) = Spc(I,J,L,id_SO4) * AERFRAC(I,J,L,1)
         Spc(I,J,L,id_SO4) = Spc(I,J,L,id_SO4) * (1.0-AERFRAC(I,J,L,1))
       ...
       IF(      State_Met%InTroposphere(I,J,L) &
         ...
         Spc(I,J,L,id_SO4) = Spc(I,J,L,id_SO4) + Spc(I,J,L,Ind_('H2SO4G'))
         Spc(I,J,L,Ind_('H2SO4G')) = 0

       ENDIF


       ! Calculate mixing ratios of other relevant species
       H2SO4GSUM = Spc(I,J,L,Ind_('H2SO4G')) * INVAIR / & !!! shw40
                  State_Chm%SpcData(Ind_('H2SO4G'))%Info%emMW_g
...
!            write(6,*)"=Total 40-bin mass [kg]:", SUM( Spc(I,J,L,id_Bins(:,1)) )
!            write(6,*)"=Total H2SO4 mass [kg]:", Spc(I,J,L,id_SO4), H2SO4SUM/1000.0
...
!	  endif !  if(I==10.and.J==23.and.L==37)then
	  ...
	  AERFRAC(I,J,L,1) = 0.0
          ...
          ! input: 
          !   r: aer_dry_rad(I_Bin)
	  !   ! bimodal parameters following APM and TOMAS schemes
          !   R1, R2
          !   Sigma1, Sigma2
...
!            end if
	    ...
	    ! APM
!            Wts1(I_Bin) = Aer_Mass(I_Bin) * Dr * 1/(Sigma1*sqrt(2.0*PI)) * 1/r &
!                       * exp( -1* (log(r)-log(R1))**2 / (2.0*Sigma1**2) )
!
!            Wts2(I_Bin) = Aer_Mass(I_Bin) * Dr * 1/(Sigma2*sqrt(2.0*PI)) * 1/r &
!                       * exp( -1* (log(r)-log(R2))**2 / (2.0*Sigma2**2) )

	    ! Grainger et al. (1995)
!            Wts1(I_Bin) = Aer_Mass(I_Bin) * Dr * 1/(Sigma1*sqrt(2.0*PI)) * 1/r &
!                       * exp( -1* (log(r)-log(R1))**2 / (2.0*Sigma1**2) ) * 22.9
!
!            Wts2(I_Bin) = Aer_Mass(I_Bin) * Dr * 1/(Sigma2*sqrt(2.0*PI)) * 1/r &
!                       * exp( -1* (log(r)-log(R2))**2 / (2.0*Sigma2**2) ) * 2.68
!
!
!          enddo
!
!          do I_Bin=1,40
	    ! APM
!            Spc(I,J,L,ID_Bins(I_Bin,1)) = Spc(I,J,L,id_SO4)*AERFRAC(I,J,L,1) * &
!				(  0.05*Wts1(I_Bin)/SUM(Wts1(:)) &
!				 + 0.95*Wts2(I_Bin)/SUM(Wts2(:)) )

	    ! Grainger et al. (1995)
!            Spc(I,J,L,ID_Bins(I_Bin,1)) = Spc(I,J,L,id_SO4)*AERFRAC(I,J,L,1) * &
!                                (  0.5*Wts1(I_Bin)/SUM(Wts1(:)) &
!                                 + 0.5*Wts2(I_Bin)/SUM(Wts2(:)) )
!          enddo
