...
!    REAL(fp)           :: MW_g
    ...
    IF(use_lagrange==10)THEN
      ...
      DO i_species=1,n_species,1
        WRITE(502,'(I10, I10, I10, E20.5)') INT(tt), INT(tt), &
...
!         IF(Plume2d%LIFE>4.0*3600.0)THEN
         IF(Plume2d%LIFE>4.0*3600.0)THEN ! !!! shw
...
!    WRITE(6,*) 'Total mass:', mass_eu, mass_la, mass_la2
    ...
    IF(Num_Plume1d+Num_Plume2d==0) use_lagrange=10
  ...
  SUBROUTINE Plume_Aerosol(am_I_Root, State_Chm, State_Grid, &
    ...
    real(fp)                    :: concnt_1D_backgrd(n_slab_max, n_bins+2)
    real(fp)                    :: concnt_1D_total  (n_slab_max, n_bins+2)
...
!      Spc_vv      = State_Chm%Species(i_lon,i_lat,i_lev,:)
	...
	concnt_1D_backgrd(:,i_species) = State_Chm%Species(i_lon,i_lat,i_lev,id_GC(i_species))
        ...
        concnt_1D_total(:,i_species) = box_concnt_1D(:,i_species) &
                        + State_Chm%Species(i_lon,i_lat,i_lev,id_GC(i_species))
...
! background
        aWP_arr         = box1d_aWP(i_slab,:) !!! shw: check the update
