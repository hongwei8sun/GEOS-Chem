...
!    REAL(fp)      :: OPTAER(State_Grid%NZ,A_)
    REAL(fp)      :: OPTAER(State_Grid%NZ,NAER*NRH) !!! shw40
...
300    FORMAT( a, ' ', a )
    ...
    NK=5
    do j=1,NAA ! = 56, from jv_spec_mie.dat 
...
!    CHARACTER(LEN=30)  :: SPECFIL_UCX(8)
    CHARACTER(LEN=30)  :: SPECFIL_UCX(47) !!! shw40
...
!                      "dust.dat"/
    ...
    "h2so4_bin_5.dat", "h2so4_bin_6.dat", "h2so4_bin_7.dat", "h2so4_bin_8.dat",&                                   
    "h2so4_bin_9.dat","h2so4_bin_10.dat","h2so4_bin_11.dat","h2so4_bin_12.dat",&                                   
    "h2so4_bin_13.dat","h2so4_bin_14.dat","h2so4_bin_15.dat","h2so4_bin_16.dat",&                                   
    "h2so4_bin_17.dat","h2so4_bin_18.dat","h2so4_bin_19.dat","h2so4_bin_20.dat",&                                   
    "h2so4_bin_21.dat","h2so4_bin_22.dat","h2so4_bin_23.dat","h2so4_bin_24.dat",&                                   
    "h2so4_bin_25.dat","h2so4_bin_26.dat","h2so4_bin_27.dat","h2so4_bin_28.dat",&                                   
    "h2so4_bin_29.dat","h2so4_bin_30.dat","h2so4_bin_30.dat","h2so4_bin_32.dat",&                                   
    "h2so4_bin_33.dat","h2so4_bin_34.dat","h2so4_bin_34.dat","h2so4_bin_36.dat",&                                   
    "h2so4_bin_37.dat","h2so4_bin_38.dat","h2so4_bin_38.dat","h2so4_bin_40.dat",&                                   
    "h2so4.dat","dust.dat"/ !!! shw: add 40 filenames here


    ! Loop over the array of filenames
    DO k = 1, NSPAA
    !!! shw40: change NSPAA's value in CMN_FJX_MOD.F90 
    !!! shw40: NSPAA index number would match diffferent species (e.g., dust)
...
110    FORMAT( 3x, a20 )
	  ...
	  !!! shw40: need to track each variable everywhere 
          READ(NJ1,*) WVAA(j,k),RHAA(i,k),NRLAA(j,i,k),NCMAA(j,i,k), &
                      RDAA(i,k),RWAA(i,k),SGAA(i,k),QQAA(j,i,k),   & !!! shw40: QQAA is dimensionless
...
!    REAL(fp),       INTENT(IN), DIMENSION(L_,A_   ) :: ODAER_COL
    REAL(fp),       INTENT(IN), DIMENSION(L_,NAER*NRH   ) :: ODAER_COL !!! shw40
...
!    REAL(fp) :: AERX_COL(A_,L1_) ! Accumulated aerosol array
    REAL(fp) :: AERX_COL(NAER*NRH+3+NDUST+NSTRATAER,L1_) ! Accumulated aerosol array !!! shw40
    ...
    do L = 1,L_
	  ...
	  !!! (1)
          ...
          DO M=1,3
             IF (AERX_COL(M,L).gt.0e+0_fp) THEN
                IDXAER=MIEDX(M) ! cloud (3, 10, 14): black carbon, water cloud, irregular ice cloud
          ...
          KMIE2=LUTIDX(KMIE) ! LUTIDX = (/1,1,2,6,10/)
	  ...
	  !!! (2)
          ...
          IF ( Input_Opt%LUCX ) THEN
             ...
             IM=10+(NRHAER*NRH)+1 !!! shw40: need to check
             !DO M=IM,IM+1
             DO M=IM,(IM+NSTRATAER) !!! shw40: 
                IDXAER=M-IM+6 !6-STS, 7-NAT !!! shw40: need to check
                ...
                IF (AERX_COL(M,L).gt.0d0) THEN
                   ...
                   DO I=1,8
                      SLEG(I,KMIE,L) = SLEG(I,KMIE,L) + &
                                       (PHAA(KMIE2,1,IDXAER,I)*LOCALSSA) !!! shw40: PAA or PHAA???
                   ENDDO     ! I (Phase function)
                ENDIF
	  ...
	  !!! (4)
          ...
          DO M=1,5 !!! 1-5: so4, soot, org, ssa, ssc
             DO IR=1,5
...
! !USES:
!
    USE CMN_SIZE_Mod,       ONLY : NAER, NRH, NSTRATAER
    USE Input_Opt_Mod,      ONLY : OptInput
...
!    REAL(fp), INTENT(IN)       :: AEROD(L_,A_)      ! Aerosol OD
    REAL(fp), INTENT(IN)       :: AEROD(L_,NAER*NRH)      ! Aerosol OD !!! shw40
...
!    REAL(fp), INTENT(OUT)      :: AERCOL(A_,L1_)    ! Aerosol column
    REAL(fp), INTENT(OUT)      :: AERCOL(NAER*NRH+3+NDUST+NSTRATAER,L1_)    ! Aerosol column !!! shw40: 46*5+3+NDUST
...
!
    ...
    IF ( Input_Opt%LUCX ) THEN !!! shw40: need to check
       ! Stratospheric aerosols - SSA/STS and solid PSCs
       MIEDX(10+(NRHAER*NRH)+1) = 4  ! SSA/LBS/STS
       MIEDX(10+(NRHAER*NRH)+2) = 14 ! NAT/ice PSCs
!       MIEDX(10+(NRHAER*NRH)+1:10+(NRHAER*NRH)+(NSTRATAER-1)) = 4  ! SSA/LBS/STS
!       MIEDX(10+(NRHAER*NRH)+NSTRATAER) = 14 ! NAT/ice PSCs
    endif
