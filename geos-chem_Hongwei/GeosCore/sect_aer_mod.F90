...
!
    ...
    !!! shw40
    Real(fp)      :: T_Col_tmp(State_Grid%NZ)
    Real(fp)      :: P_Col_tmp(State_Grid%NZ)
    Real(fp)      :: dz_Col_tmp(State_Grid%NZ)
    Real(fp)      :: air_dens_Col_tmp(State_Grid%NZ)
    Real(fp)      :: aWP_Col_tmp(State_Grid%NZ,n_aer_bin)
    Real(fp)      :: aDen_Col_tmp(State_Grid%NZ,n_aer_bin)
    ...
    !$OMP DEFAULT(  SHARED                                         ) &
    !$OMP PRIVATE(  I,          J,            K,          L            ) &
    !$OMP PRIVATE(  aWP_Box_tmp,aDen_Box_tmp, Sul_vv,     Spc_vv       ) &
    !$OMP PRIVATE(  H2O_vv,     H2SO4_vv,     H2O_vv_old, H2SO4_vv_old ) &
    !$OMP PRIVATE(  Sul_vv_old, T_K,          P_hPa,      ST_Box_tmp       ) &
    !$OMP PRIVATE(  air_dens,   CK_Box_tmp,   BVP_Box_tmp,rWet         ) &
...
!                Call AER_coagulation_Implicit(Sul_vv,fijk_box,CK_box,aWP_Box,aDen_Box,air_dens,dt_substep)
    ...
    If (Run_Micro.and.LAER_sed) Then
       !$OMP PARALLEL                                                              &
       !$OMP DEFAULT(  SHARED                                                    ) &
       !$OMP PRIVATE(  I,      J,     K,            aWP_Col_tmp, aDen_Col_tmp, Spc_Col   ) &
       !$OMP PRIVATE(  T_Col_tmp,  P_Col_tmp, air_dens_Col_tmp, dz_col_tmp,  Sul_Col,  RCOMP     )
...
!          aDen_Col => State_Chm%Strat_Den(I,J,:,:)
          ...
          aWP_Col_tmp  = State_Chm%Strat_WPcg(I,J,:,:) !!! shw40
          aDen_Col_tmp = State_Chm%Strat_Den(I,J,:,:)
          Do k=1,n_aer_bin
...
!          dz_col       => State_Met%BxHeight(I,J,:)
          T_col_tmp        = State_Met%T(I,J,:) !!! shw40
          P_col_tmp        = State_Met%PMID(I,J,:)
          air_dens_col_tmp = State_Met%AIRNUMDEN(I,J,:)
          dz_col_tmp       = State_Met%BxHeight(I,J,:)

          ! Run sedimentation in the column
          Call AER_sedimentation(Sul_Col, T_Col_tmp, P_Col_tmp, aWP_Col_tmp, aDen_Col_tmp,   & 
                                 dt_step, air_dens_col_tmp, dz_col_tmp,  NZ)
