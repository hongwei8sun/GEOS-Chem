...
!  REAL(fp), ALLOCATABLE, PUBLIC :: SLA(:,:,:)
  REAL(fp), ALLOCATABLE, PUBLIC :: SLA(:,:,:,:) !!! shw40
...
! !USES:
!
    USE CMN_SIZE_Mod,      ONLY : NRH, NRHAER, NSTRATAER !!! shw40
...
!                SLA(I,J,L) = 0e+0_fp
                SLA(I,J,L,1:NSTRATAER-1) = 0e+0_fp !!! shw40
                SPA(I,J,L) = 0e+0_fp
...
!                SLA(I,J,L) = KG_STRAT_AER(I,J,L,1) / AIRVOL(I,J,L) !!! shw40
!                SPA(I,J,L) = KG_STRAT_AER(I,J,L,2) / AIRVOL(I,J,L)
		DO I_bin = 1, NSTRATAER-1
                  SLA(I,J,L,I_bin) = KG_STRAT_AER(I,J,L,I_bin) &
						    / AIRVOL(I,J,L) !!! shw40
		ENDDO
                SPA(I,J,L) = KG_STRAT_AER(I,J,L,NSTRATAER) / AIRVOL(I,J,L)
...
!       WAERSL(:,:,:,NRHAER+1) = SLA
!       WAERSL(:,:,:,NRHAER+2) = SPA
       WAERSL(:,:,:,NRHAER+1:NRHAER+NSTRATAER-1) = SLA(:,:,:,1:NSTRATAER-1)
       WAERSL(:,:,:,NRHAER+NSTRATAER) = SPA(:,:,:)
    ENDIF
...
!       MSDENS(NRHAER+2) = 1000.0d0 ! NAT/ice PSC
       MSDENS(NRHAER+1:NRHAER+NSTRATAER-1) = 1700.0d0 ! SSA/STS
       MSDENS(NRHAER+NSTRATAER) = 1000.0d0 ! NAT/ice PSC
    ENDIF


    !!! shw40
    IF ( FIRST ) THEN
          IF ( Input_Opt%amIRoot ) WRITE( 6, 1300 )
1300      FORMAT( '     - RDAER: Finish LUCX MEDENS' )
    ENDIF
    ...
    DO IIWV = 1, NWVS
       ...
       DO N = 1, NAER
	     ...
	     !!! shw40
	     if (RW(1)==0) write(6,*) "SHW warning: denominator RW(1) is 0 in aerosol module"
             SCALER  = REFF / RW(1)
...
!          END SELECT
	  if(ISTRAT<=40) then
	    IsSLA = .TRUE.
	  else if(ISTRAT==41) then
	    IsPSC = .True.
	  else
	    WRITE( 6,'(a)') "WARNING: aerosol diagnostics not defined " // & 
	                    "for NSTRATAER greater than 41!" 
	  endif
          ...
          DO I = 1, State_Grid%NX
             ...
             ! Get aerosol effective radius !!! shw40
             ...
             !----------------------------------------------------
             ! Netcdf diagnostics computed here: !!! shw40
...
!             ENDIF
             IF ( State_Diag%Archive_AerSurfAreaSLA .AND. ISTRAT<=40 ) THEN
                State_Diag%AerSurfAreaSLA(I,J,L) = &
				State_Diag%AerSurfAreaSLA(I,J,L) + SADSTRAT
             ENDIF
             IF ( State_Diag%Archive_AerSurfAreaPSC .AND. ISTRAT==41 ) THEN
                State_Diag%AerSurfAreaPSC(I,J,L) = SADSTRAT
             ENDIF
...
!	SLA = 0.0_fp
       !!! shw40
       ALLOCATE( SLA( State_Grid%NX, State_Grid%NY, State_Grid%NZ, NSTRATAER-1 ), STAT=RC )
       CALL GC_CheckVar( 'aerosol_mod.F90:SLA', 0, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
       SLA = 0.0_fp
