...
!
  ...
  INTEGER,  PARAMETER           :: I_SPA=41 ! 2 !!! shw40
...
!
    ...
    !!! shw40: define *_40BOXs for 40-bin h2so4
    REAL(fp)                :: RAD_AER_40BOXs(40),RHO_AER_40BOXs(40)  
    REAL(fp)                :: KG_AER_40BOXs(40),NDENS_AER_40BOXs(40),SAD_AER_40BOXs(40)
    ...
    REAL(fp)		    :: GAMMA_40BOXs(11,40) !!! shw40
    INTEGER                 :: STATE_LOCAL
    ...
    !$OMP PRIVATE( RAD_AER_40BOXs, RHO_AER_40BOXs                  ) & !!! shw40
    !$OMP PRIVATE( KG_AER_40BOXs,  NDENS_AER_40BOXs, SAD_AER_40BOXs) & !!! shw40
    !$OMP PRIVATE( GAMMA_40BOXs                                    ) & !!! shw40
    !$OMP SCHEDULE( DYNAMIC )
    DO L = 1, State_Grid%NZ
    DO J = 1, State_Grid%NY
    DO I = 1, State_Grid%NX
       ...
       H2SO4_BOX_L = H2SO4SUM * AERFRAC(I,J,L,1) ! liquid
       H2SO4_BOX_G = H2SO4SUM - H2SO4_BOX_L	 ! gas

       ! Zero local properties
       RHO_AER_40BOXs   = 1000e+0_fp
       RAD_AER_40BOXs   = 0e+0_fp
       KG_AER_40BOXs    = 0e+0_fp
       NDENS_AER_40BOXs = 0e+0_fp
       SAD_AER_40BOXs   = 0e+0_fp 
	  ...
	  !!! shw40
          GAMMA_40BOXs(1,:)  = 0.1e+0_fp
          GAMMA_40BOXs(2,:)  = 0.0e+0_fp
          GAMMA_40BOXs(3,:)  = 0.0e+0_fp
          GAMMA_40BOXs(4,:)  = 0.0e+0_fp
          GAMMA_40BOXs(5,:)  = 0.3e+0_fp
          GAMMA_40BOXs(6,:)  = 0.4e+0_fp
          GAMMA_40BOXs(7,:)  = 0.9e+0_fp
          GAMMA_40BOXs(8,:)  = 0.0e+0_fp
          GAMMA_40BOXs(9,:)  = 0.0e+0_fp
          GAMMA_40BOXs(10,:) = 0.2e+0_fp
	  ...
	  RHO_AER_40BOXs   = 0.0e+0_fp
          RAD_AER_40BOXs   = 0.0e+0_fp
          KG_AER_40BOXs    = 0.0e+0_fp
          NDENS_AER_40BOXs = 0.0e+0_fp
          SAD_AER_40BOXs   = 0.0e+0_fp
	  ...
	  !!! shw40: set all SO4 as gas phase
	  !!! shw40: liquid phase will from 40-bin h2so4
          H2SO4SUM      = Spc(I,J,L,id_SO4) * INVAIR / & 
                          State_Chm%SpcData(id_SO4)%Info%emMW_g
          ...
          Do I_Bin=1, N_Aer_Bin
            ...
            KG_AER_40BOXs(I_Bin) = Mass_Bin !!! shw40: 40-bin h2so4
            ! Aerosol mass (kg) (wet)
            ...
            H2SO4_BOX_L = H2SO4_BOX_L + VV_BIN !!! shw40: need to check
	    ...
	    NDens_Aer_40Boxs(I_Bin) = NDens_Bin !!! shw40
            ...
            Gamma_40Boxs(1:11,I_Bin) = Gamma_Bin(1:11) !!! shw40

	    !!! shw40
            SAD_Aer_40Boxs(I_Bin) = SAD_Bin
            Rad_Aer_40Boxs(I_Bin) = Rad_Wet
	    !!! shw40: Mass-weighted mean (wet mass) for box density (kg/m3) 
	    Rho_Aer_40Boxs(I_Bin) = aDen(I,J,L,I_Bin)*1.0e+3_fp

          End Do ! Do I_Bin=1, N_Aer_Bin

          ! Mass-weighted mean (wet mass) for box density (kg/m3)
	  if (Vol_Sum<1.0e-30_fp) then !!! shw40
	    Rho_Aer_Box = 0.0
	  else
            Rho_Aer_Box = Mass_Sum/Vol_Sum
	  endif
          ! Total water taken up by aerosol (v/v)
          H2O_Box_L = (Mass_Sum - KG_Aer_Box) * & !!! shw40: seems no need to change
                      InvAir / H2O_MW_g
       ...
       ELSEIF (H2SO4_BOX_L.lt.1e-15_fp) THEN
          ...
          DO K=1,11
             ...
             GAMMA_40BOXs(K,:) = 0.0e+0_fp !!! shw40
          ENDDO
       ...
       ELSE
	     ...
	     CALL ERROR_STOP('This version GC code requires using 40-bin!!!', 'UCX_mod.F90')

          ELSE
             ! Ignore SLA
             DO K=1,11
                GAMMA_BOX(K) = 0.0e+0_fp
                GAMMA_40BOXs(K,:) = 0.0e+0_fp
             ENDDO
          ENDIF

	  !!! shw40: those constants need to check
	  SAD_AER_40BOXs(:)   = SAD_AER_BOX
	  RAD_AER_40BOXs(:)   = RAD_AER_BOX
	  KG_AER_40BOXs(:)    = KG_AER_BOX
	  NDENS_AER_40BOXs(:) = NDENS_AER_BOX
...
!       KHETI_SLA(I,J,L,2)  = GAMMA_BOX(2 )*KHET_SPECIFIC
       KHETI_SLA(I,J,L,I_Bin)    = GAMMA_40BOXs(1,I_Bin)*KHET_SPECIFIC !!! shw40
       KHETI_SLA(I,J,L,40+I_Bin) = GAMMA_40BOXs(2,I_Bin)*KHET_SPECIFIC !!! shw40
...
!       KHETI_SLA(I,J,L,5)  = GAMMA_BOX(5 )*KHET_SPECIFIC
       KHETI_SLA(I,J,L,80+I_Bin)  = GAMMA_40BOXs(3,I_Bin)*KHET_SPECIFIC !!! shw40
       KHETI_SLA(I,J,L,120+I_Bin) = GAMMA_40BOXs(4,I_Bin)*KHET_SPECIFIC !!! shw40
       KHETI_SLA(I,J,L,160+I_Bin) = GAMMA_40BOXs(5,I_Bin)*KHET_SPECIFIC !!! shw40
...
!       KHETI_SLA(I,J,L,7)  = GAMMA_BOX(7 )*KHET_SPECIFIC
       KHETI_SLA(I,J,L,200+I_Bin)  = GAMMA_40BOXs(6,I_Bin)*KHET_SPECIFIC !!! shw40
       KHETI_SLA(I,J,L,240+I_Bin)  = GAMMA_40BOXs(7,I_Bin)*KHET_SPECIFIC !!! shw40

       ! HOCl + HCl/HBr
       KHET_SPECIFIC= KHET_COMMON*ISR_HOCl
!       KHETI_SLA(I,J,L,8)  = GAMMA_BOX(8 )*KHET_SPECIFIC
!       KHETI_SLA(I,J,L,9)  = GAMMA_BOX(9 )*KHET_SPECIFIC
       KHETI_SLA(I,J,L,280+I_Bin)  = GAMMA_40BOXs(8,I_Bin)*KHET_SPECIFIC !!! shw40
       KHETI_SLA(I,J,L,320+I_Bin)  = GAMMA_40BOXs(9,I_Bin)*KHET_SPECIFIC !!! shw40
...
!       KHETI_SLA(I,J,L,11) = GAMMA_BOX(11)*KHET_SPECIFIC
       KHETI_SLA(I,J,L,360+I_Bin) = GAMMA_40BOXs(10,I_Bin)*KHET_SPECIFIC !!! shw40
       KHETI_SLA(I,J,L,400+I_Bin) = GAMMA_40BOXs(11,I_Bin)*KHET_SPECIFIC !!! shw40

       enddo


!       RAD_AER(I,J,L,I_SLA)  = RAD_AER_BOX*1.e+2_fp ! cm !!! shw40: constant
!       RHO_AER(I,J,L,I_SLA)  = RHO_AER_BOX      ! kg/m3
!       KG_AER(I,J,L,I_SLA)   = KG_AER_BOX       ! kg
!       NDENS_AER(I,J,L,I_SLA)= NDENS_AER_BOX    ! #/m3
!       SAD_AER(I,J,L,I_SLA)  = SAD_AER_BOX      ! cm2/cm3 !!! shw40: constant

	!!! shw40
	RAD_AER(I,J,L,I_SLA:(I_SLA+39))  = RAD_AER_40BOXs(:)*1.e+2_fp ! cm
        RHO_AER(I,J,L,I_SLA:(I_SLA+39))  = RHO_AER_40BOXs(:)    ! kg/m3
        KG_AER(I,J,L,I_SLA:(I_SLA+39))   = KG_AER_40BOXs(:)     ! kg
        NDENS_AER(I,J,L,I_SLA:(I_SLA+39))= NDENS_AER_40BOXs(:)  ! #/m3
        SAD_AER(I,J,L,I_SLA:(I_SLA+39))  = SAD_Aer_40Boxs(:)    ! cm2/cm3 
...
!
    ...
    INTEGER,INTENT(IN)          :: IAER       ! Aerosol index
                                              ! 1 = SSA (pure H2SO4)
                                              ! 2 = STS !!! shw40: really used or not?
                                              ! 3 = Solid PSC
!
! !REVISION HISTORY:
...
300       FORMAT( a, ' ', a )
    ...
    ! Indices: 1 - SO4 !!! shw40: need to check
    !          2 - HNO3
    !          3 - HCl
    !          4 - HOCl
    !          5 - HBr
