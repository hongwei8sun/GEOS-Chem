...
!  REAL(fp) :: XAREA(25), XRADI(25), XVOL(25), XH2O(25)
  REAL(fp) :: XAREA(53), XRADI(53), XVOL(53), XH2O(53) !!! shw40: nAerosol=NDUST+NAER=53
!  REAL(fp) :: KHETI_SLA(11)
  REAL(fp) :: KHETI_SLA(11,40) !!! shw40
...
!
      ...
      !!! shw40: KHETI_SLA calculated in ucx_mod using gamma
      !!! shw40: nothing (dimension) needs to change here
      !!! shw40: should add one more dimension (40-bin) to the KHETI_SLA in UCX???
      IF ( Input_Opt%LUCX ) THEN !!! shw40: KHETI_SLA related to 
!         KHETI_SLA  = State_Chm%KHETI_SLA(I,J,L,:)
         KHETI_SLA(1,:)  = State_Chm%KHETI_SLA(I,J,L,1:40)
         KHETI_SLA(2,:)  = State_Chm%KHETI_SLA(I,J,L,41:80)
         KHETI_SLA(3,:)  = State_Chm%KHETI_SLA(I,J,L,81:120)
         KHETI_SLA(4,:)  = State_Chm%KHETI_SLA(I,J,L,121:160)
         KHETI_SLA(5,:)  = State_Chm%KHETI_SLA(I,J,L,161:200)
         KHETI_SLA(6,:)  = State_Chm%KHETI_SLA(I,J,L,201:240)
         KHETI_SLA(7,:)  = State_Chm%KHETI_SLA(I,J,L,241:280)
         KHETI_SLA(8,:)  = State_Chm%KHETI_SLA(I,J,L,281:320)
         KHETI_SLA(9,:)  = State_Chm%KHETI_SLA(I,J,L,321:360)
         KHETI_SLA(10,:) = State_Chm%KHETI_SLA(I,J,L,361:400)
         KHETI_SLA(11,:) = State_Chm%KHETI_SLA(I,J,L,401:440)
      ENDIF
      ...
      !--------------------------------------------------------------------
      IF ( Input_Opt%LUCX ) THEN !!! shw40: seems nothing need to change
...
!
      ...
      DO N=1,2
         NAer = N + 10
         ! Only allow reaction on acidic aerosol !!! shw40: need to check
...
!                    		   (A**0.5_fp))
	    ...
	    !!! shw40
            ! For UCX-based mechanisms also allow reaction on stratospheric
            ! sulfate (N=13:52) if tropospheric sulfate is requested (N=8)
	    do Ibin = 1,40,1
	    kISum = kISum + ARSL1K(XAREA(12+Ibin),XRADI(12+Ibin),XDENA,B,XTEMP, &
                                  (A**0.5_fp))
	    enddo 
...
!
      ...
      DO N = 1, NAEROTYPE
         ...
         IF ( (N == 8) .OR. (N == 11) .OR. (N == 12)) THEN
            ...
            XSTKCF = B
!         ELSEIF ( N == 13 ) THEN
         ELSEIF ( N>=13 .and. N<=52 ) THEN !!! shw40
            XSTKCF = KHETI_SLA(11, N-12)
!         ELSEIF ( N == 14 ) THEN
         ELSEIF ( N == 53 ) THEN
            XSTKCF = 0.1e+0_fp
         ELSE
...
!            IF (N.eq.13) THEN ! h2so4
            IF ( N>=13 .and. N<=52 ) THEN !!! shw40
               XSTKCF = KHETI_SLA(2, N-12)
!            ELSEIF (N.eq.14) THEN
            ELSEIF (N.eq.53) THEN !!! shw40
               IF (NATSURFACE) THEN
...
!         IF (STRATBOX.and.(N.eq.13)) THEN
         IF (STRATBOX.and.(N>=13 .and. N<=52)) THEN !!! shw40
            ! Calculate for stratospheric liquid aerosol
            ! Note that XSTKCF is actually a premultiplying
            ! factor in this case, including c-bar
            ADJUSTEDRATE = XAREA(N) * XSTKCF
...
!            ELSEIF (N.eq.14) THEN
         ...
         IF (XStkCf.gt.0.0e+0_fp) THEN
!            IF (N.eq.13) THEN !!! shw40: N cannot reach 13 without UCX
            IF (N>=13.and.N<=52) THEN !!! shw40
               ! Calculate for stratospheric liquid aerosol
               ! Note that XSTKCF is actually a premultiplying
               ! factor in this case, including c-bar
