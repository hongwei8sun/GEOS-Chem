...
!
    ...
    INTEGER                :: i_box, i_ring ! Plume
    INTEGER                :: P_I, P_J, P_L ! Plume
...
!------------------------------------------------------------
    ...
    Do i_ring = 1, n_rings_max
       ...
       ! Grid-box latitude [degrees]
       YLAT      = GET_YMID( P_I, P_J, P_L )

       ! Temperature [K]
       TEMP      = State_Met%T(P_I, P_J, P_L)
...
!#endif
       ...
       ! Starting value for integration time step ???
       RCNTRL(3) = State_Chm%KPPHvalue(P_I,P_J,P_L)
...
!#endif
       ...
       ! Call the KPP integrator
       CALL Integrate( TIN,    TOUT,    ICNTRL,      &
                       RCNTRL, ISTATUS, RSTATE, IERR ) ! Plume
