...
FILEDIR2 = '/n/home12/hongwei/GC_lagrange/rundirs/geosfp_4x5_gc_timing_plume_0.1s/'
FILEDIR3 = '/n/home12/hongwei/GC_lagrange/rundirs/geosfp_4x5_gc_timing_plume_0.1s/'

model_txt  = np.loadtxt(FILEDIR+'Plume_theta_max_min_radius_5000.txt')
model2_txt = np.loadtxt(FILEDIR2+'Plume_theta_max_min_radius_5000.txt')
model3_txt = np.loadtxt(FILEDIR3+'Plume_theta_max_min_radius_5000.txt')
...
for j in range(42):
	...
	plt.plot(x, concnt_gaus, 'k.', x, model_txt[t,:], 'g-', x, model2_txt[t,:], 'r--', x, model3_txt[t,:], 'bo')
	
	plt.title( 'time = 100s * '+str(j) )

	plt.legend(('Gaussian Analytic', 'GC expansion',  'GC flux', 'GC_0.1s'),loc='upper right')
