...
geos_Ymean = np.sum(geos[:,:,:,:], axis=2)
print(geos)

lagrange = NcFile.variables['LAGRG']
lagrange_Ymean = np.sum(lagrange[:,:,:,:], axis=2)
print(lagrange)
...
while i<Nt:
	...
	fig2=ax2.pcolormesh(lon[:], lev[:], geos_Ymean[i,:,:], norm=norm, cmap="Blues")
	ax2.invert_yaxis()
	...
	norm = colors.BoundaryNorm(boundaries=bounds, ncolors=256)
	fig1=ax1.pcolormesh(lon[:], lev[:], lagrange_Ymean[i,:,:], norm=norm, cmap="Reds")
	ax1.invert_yaxis()
