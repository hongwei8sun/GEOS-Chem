...
bounds = np.linspace(1e-4,6e+14,31)
...
#
cs = m.pcolormesh(x, y, np.mean(geos_Zsum[i:i+30,:,:],axis=0), norm=norm, cmap='Reds')
...
cbar.set_label('Unit: $molec \cdot cm^2$')

plt.title('Eulerian', fontsize=14)
...
#
plt.plot(np.mean(geos_Zsum_Xmean[i:i+30,:],axis=0),lat)

a = np.mean(geos_Zsum_Xmean[i:i+30,:],axis=0)
print( 0.5*np.mean(a[8:16],axis=0) + 0.5*np.mean(a[30:38],axis=0) )
print( np.mean(a[16:30],axis=0) )
print( np.mean(a[16:30],axis=0) - 0.5*np.mean(a[8:16],axis=0) - 0.5*np.mean(a[30:38],axis=0) )

X_max = 5.0e+14
plt.xlim(0,X_max)
plt.ylim(-90,90)
plt.xlabel('Tracer Concentration [$molec \cdot cm^2$]')
...
#
cs = m.pcolormesh(x, y, np.mean(lagrange_Zsum[i:i+30,:],axis=0), norm=norm, cmap='Reds')
...
plt.title('Lagrangian', fontsize=14)
...
#
plt.plot(np.mean(lagrange_Zsum_Xmean[i:i+30,:],axis=0),lat)

a = np.mean(lagrange_Zsum_Xmean[i:i+30,:],axis=0)
...
plt.savefig('Month_mean_xy2.png')
plt.clf()
plt.cla()
