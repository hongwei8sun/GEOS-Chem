...
MODULE Plume_Mod
  ...
  real(fp), allocatable :: box_radiusA(:,:)   ! a, vertical radius
  real(fp), allocatable :: box_radiusB(:,:)   ! b, horizontal radius of the the cross-section
  ...
  real(fp), allocatable :: box_concntA(:,:)      ! [kg/m3], box_concnt(n_boxes_max,N_rings)
  real(fp), allocatable :: box_concntB(:,:)      ! [kg/m3], box_concnt(n_boxes_max,N_rings)
  real(fp), allocatable :: box_concnt_std(:,:)      ! [kg/m3], box_concnt(n_boxes_max,N_rings)
  real(fp), allocatable :: box_concntA_old(:,:)  ! [kg/m3], box_concnt(n_boxes_max,N_rings)
  real(fp), allocatable :: box_concntB_old(:,:)  ! [kg/m3], box_concnt(n_boxes_max,N_rings)
  ...
  ! Init_amount = Total_amount / box_length
  real(fp), allocatable :: Init_amount   ! [kg/m], the initial amount of chemicals
...
!---------------------------------------------------------------------
  ...
  SUBROUTINE plume_init( am_I_root )
    ...
    allocate(box_radiusA_big(n_boxes_max,n_rings_max))
    allocate(box_radiusB_big(n_boxes_max,n_rings_max))
    allocate(box_radiusA_small(n_boxes_max,n_rings_max))
    allocate(box_radiusB_small(n_boxes_max,n_rings_max))
    ...
    allocate(box_concntA(n_boxes_max,n_rings_max))
    allocate(box_concntB(n_boxes_max,n_rings_max))
    allocate(box_concnt_std(n_boxes_max,n_rings_max))
    allocate(box_concntA_old(n_boxes_max,n_rings_max))
    allocate(box_concntB_old(n_boxes_max,n_rings_max))
    ...
    do i_ring=1,n_rings_max
      ...
      box_concntA(:,i_ring) = (/0.0e+0_fp,  0.0e+0_fp,  0.0e+0_fp/)   ! [kg/m3]
      box_concntB(:,i_ring) = (/0.0e+0_fp,  0.0e+0_fp,  0.0e+0_fp/)   ! [kg/m3]
    ...
    do i_box = 1,n_boxes_max
       ...
       do t1s=1,int(Dt)
         ...
         do i_ring = 1, n_rings_max
           ...
           box_concntA_old(i_box,i_ring) = box_concntA(i_box,i_ring)
           box_concntB_old(i_box,i_ring) = box_concntB(i_box,i_ring)
        ...
        ! Change the box_radiusA used here !!!
         ...
         do i_ring = 1,n_rings_max
           box_concnt_std(i_box,i_ring) = box_concntA(i_box,i_ring) * box_concntB(i_box,i_ring)
           box_concnt(i_box,i_ring) = Init_amount * box_concnt_std(i_box,i_ring)
         enddo       
...
!         env_amount(i_box) = env_amount(i_box) + box_concnt(i_box,n_rings_max) * box_Dvolumn_big(i_box,n_rings_max)
         ...
         write(262,*) box_concntA(1,:)
         write(262,*) box_concntB(1,:)
         write(262,*) box_concnt(1,:)
