...
!C  AIR IS DIAMETER OF AN AIR MOLECULE, SULFATE IS DIAMETER OF AN H2SO4
    AIR=2.*(28.9/((4./3.)*PI*DEN1*AV))**(1./3.)
    SULFATE=2.*(98./((4./3.)*PI*1.8409*AV))**(1./3.)
    D=0.5*(AIR+SULFATE)
...
!   DIFF IS THE DIFFUSION COEFFICIENT
    DIFF=3./(8.*D**2*AD)*SQRT(AKB*Temp/(2.*PI*SULF)* &
         ((28.9+98.)/28.9)) ! shw
...
!         write(6,*)'bin, RAD, AD, D, EFF, (1.+CORR*VKNUD), 4.*PI*RAD:'
!         write(6,*)K, RAD, AD, D, EFF, (1.+CORR*VKNUD), 4.*PI*RAD
