...
! Version 9 (Nov15, 2020):
! This version include the shadow plume model
! In this version, shadow plume model has interaction with background grid cell,
! which is not correct.
! It easy to cut the interaction from shadow plume into background grid cell,
! but it's not easy to cut the interaction from the backgound grid cell into the
! shadow plume.
...
! Check combine 9 to 1 grid cell is correct.
...
MODULE Lagrange_Mod
  ...
  PUBLIC :: box_concnt_2D ! (n_box, n_x_max, n_y_max, N_specie)
...
!  integer, parameter    :: n_box = 60 !6904224     
  ...
  real(fp), allocatable :: box_concnt_1D(:,:,:)   !(n_box,N_slab,N_specie)
  real(fp), allocatable :: box_concnt_2D(:,:,:,:) !(n_box,N_slab,N_specie)
...
! add new box every time step
    ...
    allocate(Data2D(n_box_prev, N_specie))
    allocate(Data3D(n_box_prev, n_slab_max, N_specie))
    allocate(Data4D(n_box_prev, n_x_max, n_y_max, N_specie))
...
!       curr_pressure = box_lev(i_box) + dbox_lev
      ...
      box_u(i_box)    = ( RK_u(1) + 2.0*RK_u(2) &
                         + 2.0*RK_u(3) + RK_u(4) ) / 6.0
      box_v(i_box)    = ( RK_v(1) + 2.0*RK_v(2) &
      ...
      i_lon = Find_iLonLat(box_lon(i_box), Dx, X_edge2)
      if(i_lon==IIPAR+1) i_lon=1

      i_lat = Find_iLonLat(box_lat(i_box), Dy, Y_edge2)
      ...
      IF(i_box==1.and.Judge_plume(i_box)==2) WRITE(6,*) 0, '2D', i_box, &
              ...
              SUM(Extra_mass_2D(i_box,2:n_x_max-1,2:n_y_max-1,1))*V_new

      IF(i_box==1.and.Judge_plume(i_box)==1) WRITE(6,*) 0, '1D', i_box, &
              SUM(box_concnt_1D(i_box,2:n_slab_max-1,1))*V_new, &
              SUM(Extra_mass_1D(i_box,2:n_slab_max-1,1))*V_new
...
!---------------------------------------------------------------------
  ...
  SUBROUTINE plume_run(am_I_Root, State_Chm, State_Met, Input_Opt, RC)
    ...
    ! toward travel direction) and lon: box_alpha belongs to [-PI, PI)
    ! box_alpha = beta - PI/2
    real(fp)          :: beta, box_alpha ! [radians]
...
!                WRITE(6,*) '*** ERROR 3 ***', eddy_v, Pdt, Pdy(i_box)
         ...
         IF(i_box==2095.and.t1s==1) WRITE(6,*) 1, i_box, Lifetime(i_box), &
...
!         ENDDO
         ...
         box_concnt_2D(i_box,2:n_x_max-1,2:n_y_max-1,1) =               &
                Pc(2:n_x_max-1,2:n_y_max-1)                             &
           + Pdt*( eddy_h*(   Pc(1:n_x_max-2,2:n_y_max-1)               &
                           -2*Pc(2:n_x_max-1,2:n_y_max-1)               &
                             +Pc(3:n_x_max,2:n_y_max-1)  )              &
                                                /(Pdx(i_box)**2)        &
                 + eddy_v*(   Pc(2:n_x_max-1,1:n_y_max-2)               &
                           -2*Pc(2:n_x_max-1,2:n_y_max-1)               &
                             +Pc(2:n_x_max-1,3:n_y_max)  )              &
                                                /(Pdy(i_box)**2) )
...
!           box_theta(i_box) = 1.521235 !!! shw
           ...
           D_mass_plume = V_grid_1D * SUM(box_concnt_1D(i_box,2:n_slab_max-1,1)) &
                 - V_grid_2D * SUM(box_concnt_2D(i_box,2:n_x_max-1,2:n_y_max-1,1))
...
!           WRITE(6,*) i_box, Pdx(i_box), Pdy(i_box), box_length(i_box), &
!                      SUM(box_concnt_2D(i_box,2:n_x_max-1,2:n_y_max-1,1)), &
!                      SUM(Extra_mass_2D(i_box,2:n_x_max-1,2:n_y_max-1,1))
...
!                   -SUM(Extra_mass_2D(i_box,2:n_x_max-1,2:n_y_max-1,1)) ) ) &
!                        >0.01)THEN
!                WRITE(6,*)"WARNING: mass is not conserved when 2D to 1D:", &
!                                i_box, D_mass_plume &
!                 /( Pdx(i_box)*Pdy(i_box)*box_length(i_box)*1.0e+6_fp &
!                * ( SUM(box_concnt_2D(i_box,2:n_x_max-1,2:n_y_max-1,1)) &
!                   -SUM(Extra_mass_2D(i_box,2:n_x_max-1,2:n_y_max-1,1)) ) )
...
!                                     / COS(box_lat(i_box)/180*PI)**2 ) )
         ...
         DlonN = ABS(box_Ra(i_box)*COS(box_alpha)/N_split) / &
                        (2*PI *(Re*COS(box_lat(i_box)/180*PI)) ) * 360

         DlatN = ABS(box_Ra(i_box)*SIN(box_alpha)/N_split) / (2*PI*Re) * 360
...
!            / (SIN(y1)*SIN(y2)+COS(y1)*COS(y2)*COS(x2-x1)) )
  ...
  real(fp) function Wind_shear_s(u, v, P_BXHEIGHT, box_alpha, i_lon, i_lat, i_lev, curr_lon, curr_lat, curr_pressure)
    implicit none
    real(fp)          :: curr_lon, curr_lat, curr_pressure
    real(fp)          :: box_alpha
    !real(fp), pointer :: PI, Re
    ...
    do k = 1,2
      ...
      wind_s(k) = u_lonlat(k)*COS(box_alpha) + v_lonlat(k)*SIN(box_alpha)
