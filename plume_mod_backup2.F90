    ...
    do i_box = 1,n_boxes_max
       ...
       do t1s=1,int(Dt)
         ...
         box_concnt(i_box,1) = box_concnt_old(i_box,1) &
                             + box_concnt_new(i_box,2) * box_Dvolumn_small(i_box,1)/box_volumn(i_box,1) &
                             - box_concnt_new(i_box,1) * box_Dvolumn_big(i_box,1)/box_volumn(i_box,1)
         ...
         do i_ring = 2, n_rings_max-1
           box_concnt(i_box,i_ring) = box_concnt_old(i_box,i_ring) &
                   ...
                   + box_concnt_new(i_box,i_ring+1) * box_Dvolumn_small(i_box,i_ring)/box_volumn(i_box,i_ring) &
                   ! input from i_ring-1 to i_ring:
                   + box_concnt_new(i_box,i_ring-1) * box_Dvolumn_big(i_box,i_ring-1)/box_volumn(i_box,i_ring) &
                   ! output from i_ring to i_ring-1 and i_ring+1:
                   - box_concnt_new(i_box,i_ring) * ( box_Dvolumn_big(i_box,i_ring) + box_Dvolumn_small(i_box,i_ring-1) )/box_volumn(i_box,i_ring)
         enddo

         ! For outest ring:
         box_concnt(i_box,n_rings_max) = box_concnt_old(i_box,n_rings_max) &
         ! input from n_rings_max-1 to n_rings_max:
         + box_concnt_new(i_box,n_rings_max-1) * box_Dvolumn_big(i_box,n_rings_max-1)/box_volumn(i_box,n_rings_max) &
         ! output from n_rings_max to n_rings_max-1 and environment:
         - box_concnt_new(i_box,n_rings_max) * (box_Dvolumn_big(i_box,n_rings_max)+box_Dvolumn_small(i_box,n_rings_max-1))/box_volumn(i_box,n_rings_max)
