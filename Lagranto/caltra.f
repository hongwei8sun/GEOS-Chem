...
c     Meteorological fields
      ...
      real,allocatable, dimension (:)     :: temp0,temp1     ! 3d temperature
...
C     Allocate memory for some meteorological arrays
      ...
      allocate(temp0(nx*ny*nz),stat=stat)
      if (stat.ne.0) print*,'*** error allocating array temp0 ***'  ! temperature
      allocate(temp1(nx*ny*nz),stat=stat)
      if (stat.ne.0) print*,'*** error allocating array temp1 ***'
      allocate(tht0(nx*ny),stat=stat)
...
C     jump	 input  flag (=1 trajectories donot enter the ground)
...
C        Calculate new winds for advection
     ...
     >                    nx,ny,nz,xmin,ymin,dx,dy,mdv)
         ...
         T_K1 = int_index4(temp0,temp1,nx,ny,nz,xind,yind,pind,reltpos1,mdv) !!! shw
