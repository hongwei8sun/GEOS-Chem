...
C   any damages that may result from the use of the program. 
     ...
     &       BET2(NPL),MRR,MRI,LAM   
      ...
      INTEGER, PARAMETER :: NUM=287, Nbin=1
      INTEGER*4 inum
      ...
      open (261, file='h2so4_GC_old.dat',
     * status='old', access='sequential', form='formatted')
      ...
      READ(261,'(f7.1, f5.0, f8.4, E11.3, f7.3, f7.3)')
     * LAM_array(inum), RH(inum), MRR_array(inum),
     * MRI_array(inum), AA_dry(1,inum), AA_array(1,inum)
...
!     * MRI_array(inum), AA_dry(1:40,inum), AA_array(1:40,inum)
      ...
      ibin=1
        ...
        open(484, file='h2so4_GC_new.dat',
...
C        R2=AA*1.0000001 D0
      ...
      AA=DBLE(AA_array(ibin,inum))	! r_g: radius [um]
      BB=0.2609428			! [ln(sigma_g)]**2, sigma_g=0.6
!      AA1=0.17D0
!      AA2=3.44D0
!      BB1=DLOG(1.96D0)*DLOG(1.96D0)
!      BB2=DLOG(2.37D0)*DLOG(2.37D0)
      GAM=1D0                                                               
      LAM=DBLE(LAM_array(inum))/1000.0	! wavelength [um]
 ...
 1000 FORMAT ('NUMERICAL ACCURACY =',D8.2)
     ...
     *       RMEAN,AA1,BB1,AA2,BB2,DDELT, ALB, REFF)
      QE=CEXT/AREA
      LMAX=L1-1
 ...
 1001 FORMAT ('<G> = ',D12.6,'  <V> = ',D12.6,'  Rvw = ',D12.6,
     *        '  <R> = ',D12.6)
      ...
      WRITE(484, 1074 )
     *	LAM, INT(RH(inum)), MRR, MRI, AA_dry(ibin,inum), AA,
     *   0.6, QE, 0.99,   REFF,  ALB, AL1(2)/3D0, AL1(1:8)
...
C***************************************************************
     ...
     *            VOLUME,RVW,RMEAN,AA1,BB1,AA2,BB2,DDELT, ALB, REFF)
...
!      WRITE(484, '(f8.1, f8.3, f8.3, 8f8.3)') LAM, CEXT, ALB, AL1(1:8)
