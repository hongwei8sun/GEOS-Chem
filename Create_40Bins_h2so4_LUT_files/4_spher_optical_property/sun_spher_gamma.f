...
C   any damages that may result from the use of the program. 
     ...
     &       BET2(NPL),MRR,MRI,LAM   
      ...
      INTEGER, PARAMETER :: NUM=287, Nbin=40
      INTEGER*4 inum
      REAL*8 LAM_array(NUM), MRR_array(NUM), MRI_array(NUM)
      REAL*8 AA_array(Nbin,NUM), AA_dry(Nbin,NUM), RH(NUM)
      CHARACTER*1 BIN_1           
      CHARACTER*2 BIN_2           


      open (6, file='SUN_spher.print')
      open (10, file='SUN_spher.write')

      open (261, file='h2so4_input_40bins.txt',
...
!     * MRI_array(inum), AA_dry(1:40,inum), AA_array(1:40,inum)
      ...
      IF(ibin<10)THEN
        ...
        WRITE(BIN_1, "(I1)") ibin
        open(484, file=TRIM('h2so4_bin_'//BIN_1//'.dat'),
     *  status='replace', access='sequential', form='formatted')

      ELSE 

        WRITE(BIN_2, "(I2)") ibin
        open(484, file=TRIM('h2so4_bin_'//BIN_2//'.dat'),
     *  status='replace', access='sequential', form='formatted')
     ...
     * "h2so4 RRTMG LUT interpolated from GADS (DAR 09/2013)"
      WRITE(484,*)
     *  "V0.1 - contains Fast-J wavelengths but is the same as the ",
     1  "original jv_spec.dat values"
      WRITE(484,*)
     * " w(nm)   RH       N         iK  RgDry  RgWet      Sg    ",
     1 "   Q  Alpha  r-eff ss-alb  g-sym  pi(0)  pi(1)  pi(2)  ",
     2 "pi(3)  pi(4)  pi(5)  pi(6)  pi(7)"

      DO inum=1,NUM,1


      IF(MRI_array(inum)<0) MRI_array(inum) = -1.0*MRI_array(inum)
...
C        R2=AA*1.0000001 D0
      ...
      AA=DBLE(AA_array(ibin,inum))	! r_g: radius
      BB=1D-1			! [ln(sigma_g)]**2
      AA1=0.17D0
      AA2=3.44D0
      BB1=DLOG(1.96D0)*DLOG(1.96D0)
      BB2=DLOG(2.37D0)*DLOG(2.37D0)
      GAM=1D0                                                               
      LAM=DBLE(LAM_array(inum))/1000.0	! wavelength                                        
      MRR=DBLE(MRR_array(inum))	! refractie index: real                                               
      MRI=DBLE(MRI_array(inum))	! refractive index: imaginary
      NDISTR=4	
      NK=1
      N=1
      NP=4
      R1=AA*0.9999999 D0	! min
      R2=AA*1.0000001 D0	! max
      DDELT=1D-7
 ...
 1000 FORMAT ('NUMERICAL ACCURACY =',D8.2)
     ...
     *       RMEAN,AA1,BB1,AA2,BB2,DDELT, ALB)
      QE=CEXT/AREA
      LMAX=L1-1
 ...
 1001 FORMAT ('<G> = ',D12.6,'  <V> = ',D12.6,'  Rvw = ',D12.6,
     *        '  <R> = ',D12.6)
      ...
      WRITE(484, 1074 )
     *	LAM*1000, INT(RH(inum)), MRR, MRI, AA_dry(ibin,inum), AA,
     *   0.6, QE, 0.99,   AA,  ALB, AL1(2)/3D0, AL1(1:8)
 1074 FORMAT (f7.1, I5, f8.4, E11.3, f7.3, f7.3
     *  f8.2, f8.4, f7.3, f7.3, f7.3,       f7.3,   8f7.3)
! 1074 FORMAT (f7.1, I5, f8.4, E11.3, f9.5, f9.5
!     *  f8.2, E11.3, f7.3, f7.3, f7.3,       f7.3,   8E11.3)

      END DO ! DO inum=1,NUM,1

      WRITE(*,*)"Finish: ibin=", ibin
      CLOSE(484)

      END DO ! DO ibin=1,Nbin,1
...
C***************************************************************
     ...
     *            VOLUME,RVW,RMEAN,AA1,BB1,AA2,BB2,DDELT, ALB)              
...
C      WRITE(484, '(f8.1, f8.3, f8.3, 8f8.3)') LAM, CEXT, ALB, AL1(1:8)
